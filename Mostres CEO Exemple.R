...
#Carreguem les dades electorals
...
#Ara podem generar el codi de 10 dígits que identifica les seccions
...
#Treiem el CERA, que està com una fila en cada província
dades_seccions21<-dades_seccions21%>%
  filter(Cens<10000)

#Ara calculem sis Clusters per vot 2021 utilitzant l'algoritme K-means
set.seed(3456)
...
# Afegim l'identificador de cluster de cada secció a les dades electorals
dades_seccions21$cluster21 <- as.factor(cluster21$cluster)

#EXTREIEM UNA MOSTRA ESTRATIFICADA PER TIPOLOGIA DE SECCIÓ, 
#Amb una probabilitat d'inclusió PROPORCIONAL A LA MIDA de la secció

#Definim el número total de seccions a extreure

n<-169

#Cal definir quantes seccions s'extreuen per estrat.

# Calculem la proporció del cens que viu a cada estrat
...
#multipliquem la proporció per la n total
resumestratscluster$seccions<-resumestratscluster$Propcens*n
resumestratscluster
...
csample <- dadesc[ppssstrat(dadesc$Cens,dadesc$cluster21,nsecestratcluster),] #extreiem la mostra amb ppssstrat

#Avaluem mostres

#Quants municipis diferents es visiten?
...
datasummary_balance(~selected.csample, data = dplyr::select(dades_seccions21, selected.csample, pabst21, erc.vots21, psc.vots21, cup.vots21, pp.vots21, comuns.vots21, cs.vots21, junts.vots21, vox.vots21), dinm_statistic = "p.value", fmt =  '%.3f') 

#guardem la taula en un html
...
##Analitzem les mostres amb altres dades sociodemogràfiques

#Afegim les dades de renda, provinents de l'INE
...
seccions_select <- dades_seccions21 %>% 
  dplyr::select(CUSEC, cluster21, selected.csample) 
